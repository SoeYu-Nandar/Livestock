...
<x-layout>
    <div class="container my-3">
      ...
      <div class="row">
        <div class="col-md-4">
          <img src="/img/wfood.jpg" alt="" class="img-fluid rounded" style="height:300px;">
        </div>
        <div class="col-md-8">
          <div class="card-body">
            <div class="card-title fw-bold">နွားစာများ</div>
              <p class="card-text">မွေးမြူရေးလုပ်ငန်းရှင်များသည်စိန်ခေါ်မှုအပြည့်ရှိနေတဲ့ပတ်ဝန်းကျင်အနေအထားများမှာလုပ်ငန်းလုပ်ကိုင်လျက်ရှိနေကြပါသည်။
                ...
                ကျွန်ုပ်တို့မှကူညီဆောင်ရွက်ပေးလျက်ရှိနေပါသည်။မိတ်ဆွေတို့နွားခြံအတွက် အစားအစာ၊ အဟာရဆိုင်ရာစီမံခန့်ခွဲမှုများနှင့်အခြားသော ရှု့ထောင့်ပေါင်းစုံမှလိုအပ်သောစီမံခန့်ခွဲမှုများအတွက်အမြဲကူညီဖို့အသင့်ရှိပါသည်။
              </p> 
          </div>
        </div>
      </div>
  ...
  <div class="dropdown mb-2 mt-2 text-center">  
    ...
    <ul class="dropdown-menu">
      <li><a class="dropdown-item" href="/cow_foods">အားလုံး</a></li>
  @foreach ($companies as $company)
  <li><a class="dropdown-item" href="/cow_foods/?company={{$company->slug}}{{request('search')?'&search='.request('search') : ''}}">{{$company->name}}</a></li>
  @endforeach
    </ul>
  </div> 
    ...
    <div class="input-group my-4">
      ...
      @endif              
        ...
        <div class="row text-center">
            @forelse ($cowfoods as $cowfood)
                <div class="col-md-4 mb-4">
                  <div class="card m-5 text-center overflow-hidden" style="width:18rem;border:none;">
                    <img src="/storage/{{$cowfood->image}}" class="card-img-top" alt="..." style="height:250px;width:250px;">
                    <div class="card-body">
                      <h5 class="card-title">{{$cowfood->code}}</h5>   
                      <div class="tags mb-2">
                        <a href="/cow_foods/?company={{$cowfood->company->slug}}"><span class="badge text-bg-dark">{{$cowfood->company->name}}</span></a>
                  </div>
                      <p class="card-text text-muted">{{$cowfood->price}} ကျပ်</p>
                      <a href="/cow_foods/{{$cowfood->id}}" class="btn btn-primary rounded-pill">ပိုမိုလေ့လာမည်</a>
                    </div>
                  </div>
                </div>
                @empty
                <p class="text-center">No cow livestock food found.</p>
                @endforelse
                {{$cowfoods->links()}}
        </div>
</section>
